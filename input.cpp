// Utility: shrink a surface by integer factors (width/height divisors)
SDL_Surface* shrinkSurface(SDL_Surface* src, int factorW, int factorH) {
    int newW = src->w / factorW;
    int newH = src->h / factorH;
    if (newW < 1) newW = 1;
    if (newH < 1) newH = 1;
    SDL_Surface* dst = SDL_CreateRGBSurfaceWithFormat(0, newW, newH, 32, SDL_PIXELFORMAT_RGBA8888);
    if (!dst) return src;
